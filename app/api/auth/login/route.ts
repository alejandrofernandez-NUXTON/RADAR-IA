...
export async function POST(request: NextRequest) {
  ...
  const identifier = String(formData.get("user") || formData.get("email") || "").toLowerCase().trim();
  ...
  const envFallbackOk =
    !user &&
    identifier === (process.env.ADMIN_EMAIL || "").toLowerCase() &&
  ...
  const token = await createSessionToken(user?.email || identifier, process.env.NEXTAUTH_SECRET || "");
